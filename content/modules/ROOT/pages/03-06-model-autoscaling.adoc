...
* After starting the 3-06 notebook, you can continue with the next instructions in order to watch your deployment autoscale.
* The first time sending this many requests will fail, and the deployment will spin up more replicas.
...
Once the post requests are made. The pod will begin working, but fails when it recieves too large of a workflow. To solve this, it spins up more deployments to handle the increased workload.
